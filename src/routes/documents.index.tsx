...
import { mockDocuments, type MockDocument } from "@/lib/mock-data";
...
function DocumentsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (q) list = list.filter((d) => d.title.toLowerCase().includes(q.toLowerCase()) || d.tags.some((t) => t.toLowerCase().includes(q.toLowerCase())));
  ...
  const clearFilters = () => {
    setQ(""); setStatus("all"); setType("all"); setTag("all"); setSort("updated");
  };
      ...
      <div className="mx-auto max-w-[1280px] px-4 py-8 lg:px-8">
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1.5 text-sm text-text-secondary">Manage every source in your workspace.</p>
          </div>
          <Link to="/upload">
        ...
        <div className="mt-8 flex flex-wrap items-center gap-2 rounded-xl border border-border bg-surface p-3">
          <div className="relative min-w-[220px] flex-1">
            ...
            <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search by title or tag..." className="h-9 pl-8" />
          </div>
          <FilterSelect label="Status" value={status} onChange={setStatus} options={[["all","All"],["ready","Ready"],["processing","Processing"],["failed","Failed"]]} />
          <FilterSelect label="Type" value={type} onChange={setType} options={[["all","All"],["PDF","PDF"],["DOCX","DOCX"],["TXT","TXT"]]} />
          <FilterSelect label="Tag" value={tag} onChange={setTag} options={[["all","All"],["Research","Research"],["Career","Career"],["Legal","Legal"],["Product","Product"],["Important","Important"]]} />
          <FilterSelect label="Sort" value={sort} onChange={setSort} options={[["updated","Recently updated"],["name","Name"],["questions","Questions"]]} />
          <div className="ml-auto flex items-center gap-1 rounded-md border border-border bg-background p-0.5">
            <button onClick={() => setView("table")} className={`flex h-7 w-7 items-center justify-center rounded ${view==="table"?"bg-surface text-foreground":"text-text-muted"}`}>
              <List className="h-3.5 w-3.5" />
            </button>
            <button onClick={() => setView("grid")} className={`flex h-7 w-7 items-center justify-center rounded ${view==="grid"?"bg-surface text-foreground":"text-text-muted"}`}>
        ...
        {selected.length > 0 && (
          <div className="mt-3 flex items-center gap-3 rounded-md border border-accent-border/60 bg-accent-soft px-4 py-2 text-sm">
            ...
            <div className="ml-auto flex items-center gap-1">
              <Button size="sm" variant="ghost" className="h-8 gap-1.5"><Tag className="h-3.5 w-3.5" />Add tag</Button>
              <Button size="sm" variant="ghost" className="h-8">Export</Button>
              <Button size="sm" variant="ghost" className="h-8 gap-1.5 text-destructive hover:text-destructive"><Trash2 className="h-3.5 w-3.5" />Delete</Button>
              <Button size="sm" variant="ghost" className="h-8 gap-1.5" onClick={() => setSelected([])}><X className="h-3.5 w-3.5" />Cancel</Button>
            </div>
          </div>
        )}
            ...
            hasFilters ? (
              <div className="rounded-xl border border-dashed border-border bg-surface p-16 text-center">
                <div className="text-sm text-text-secondary">
                  No documents found for <span className="font-medium text-foreground">"{q || "these filters"}"</span>
                </div>
                <p className="mt-2 text-xs text-text-muted">Try another keyword or clear filters.</p>
                <Button onClick={clearFilters} variant="outline" className="mt-5">Clear filters</Button>
              </div>
            ) : (
              <EmptyDocs />
            )
...
function TableView({ docs, selected, setSelected }: { docs: MockDocument[]; selected: string[]; setSelected: (s: string[]) => void }) {
    ...
    <div className="overflow-hidden rounded-xl border border-border bg-surface">
      <table className="w-full text-sm">
        <thead className="border-b border-border bg-surface-soft text-xs text-text-secondary">
          <tr>
            <th className="w-10 px-4 py-2.5"><Checkbox checked={selected.length === docs.length && docs.length > 0} onCheckedChange={toggleAll} /></th>
        ...
        <tbody className="divide-y divide-border">
          {docs.map((d) => (
            <tr key={d.id} className="hover:bg-surface-soft">
              <td className="px-4 py-3"><Checkbox checked={selected.includes(d.id)} onCheckedChange={() => toggle(d.id)} /></td>
              <td className="px-2 py-3">
                <Link to="/documents/$slug" params={{ slug: d.slug }} className="flex items-center gap-3">
              ...
              <td className="hidden px-4 py-3 md:table-cell">
                <div className="flex flex-wrap gap-1">
                  {d.tags.map((t) => (
                    <span key={t} className="rounded-full border border-border bg-background px-2 py-0.5 text-[11px] text-text-secondary">{t}</span>
                  ))}
                </div>
              </td>
              <td className="hidden px-4 py-3 font-mono text-xs text-text-secondary lg:table-cell">{d.size}</td>
...
function GridView({ docs }: { docs: MockDocument[] }) {
  return (

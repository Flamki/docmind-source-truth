...
import { mockDocuments, mockMessages, type Citation, type ChatMessage } from "@/lib/mock-data";
...
export const Route = createFileRoute("/documents/$slug")({
  loader: ({ params }) => {
    const doc = mockDocuments.find((d) => d.slug === params.slug);
    if (!doc) throw notFound();
    return { doc };
  },
  head: ({ loaderData }) => ({
    meta: [{ title: loaderData ? `${loaderData.doc.title} — DocMind` : "Not found" }],
  }),
  notFoundComponent: () => (
    <AppShell breadcrumb={[{ label: "Documents", to: "/documents" }, { label: "Not found" }]}>
      <div className="mx-auto max-w-md px-6 py-24 text-center">
        <h1 className="text-2xl font-semibold">Document not found</h1>
        <p className="mt-2 text-sm text-text-secondary">
          The document may have been deleted or the URL is wrong.
        </p>
        <Link to="/documents"><Button className="mt-6">Back to documents</Button></Link>
      </div>
    </AppShell>
  ),
...
function DocumentDetail() {
  ...
  const [selectedCitation, setSelectedCitation] = useState<Citation | null>(
    mockMessages[1]?.citations?.[0] ?? null,
  );
        ...
        <div className="lg:hidden">
          <Tabs defaultValue="chat">
            ...
            <TabsContent value="chat" className="mt-4">
              <ChatPanel doc={doc} messages={messages} setMessages={setMessages} onCitation={setSelectedCitation} />
            </TabsContent>
...
function DetailsPanel({ doc }: { doc: typeof mockDocuments[number] }) {
  return (
    <aside className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        ...
        <div className="mt-2 flex flex-wrap gap-1.5">
          {doc.tags.map((t) => (
            <span key={t} className="rounded-full border border-border bg-background px-2 py-0.5 text-xs text-text-secondary">{t}</span>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        ...
        <ul className="mt-2 space-y-1">
          {["Summarize this document", "What are the risks?", "What are the deadlines?", "Who is responsible?"].map((q) => (
            <li key={q}>
  ...
  const submit = () => {
    ...
    setTimeout(() => {
      const answer: ChatMessage = {
        ...
        role: "assistant",
        content:
          "Based on the document, the key considerations are outlined across sections on scope, timeline, and risk. Ownership is distributed across product, engineering, and design leads.",
        citations: [
          { id: 1, page: 2, title: "Scope", snippet: "The initial scope covers upload, search, and citation flows.", relevance: "High" },
          { id: 2, page: 5, title: "Timeline", snippet: "A four-week window is allocated for the closed beta cohort.", relevance: "Medium" },
        ],
      };
  ...
  return (
    <section className="flex min-h-[calc(100vh-8rem)] flex-col rounded-xl border border-border bg-surface">
      <div className="flex items-center gap-2 border-b border-border px-5 py-3">
        ...
        <div className="text-sm font-medium">Ask <span className="text-text-secondary">{doc.title}</span></div>
      </div>
      ...
      <div className="border-t border-border p-3">
        <div className="rounded-lg border border-border bg-background p-2 focus-within:border-text-muted">
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <Button size="sm" onClick={submit} disabled={!input.trim() || loading} className="h-8 gap-1.5">
  ...
  if (m.role === "user") {
    return (
      <div className="flex justify-end">
        <div className="max-w-[75%] rounded-lg rounded-br-sm bg-surface-soft px-4 py-2.5 text-sm">{m.content}</div>
      </div>
    );
  }
  return (
        ...
        <div className="flex flex-wrap gap-1.5">
          {m.citations.map((c) => (
            ...
            >
              ...
              [{c.id}] Page {c.page} · {c.title}
            </button>
          ))}
        </div>
      )}
      <div className="flex flex-wrap gap-1.5 pt-1">
        {["What should be fixed first?", "Turn this into an action plan", "Who owns each risk?"].map((s) => (
          <button key={s} className="rounded-full border border-border bg-background px-3 py-1 text-xs text-text-secondary hover:border-text-muted hover:text-foreground">
            {s}
          </button>
        ))}
      </div>
...
function ChatEmpty({ onPick }: { onPick: (v: string) => void }) {
  const qs = ["Summarize in 5 bullets", "Find deadlines", "List open questions", "Explain like I'm new"];
  return (
    <div className="mx-auto max-w-md py-12 text-center">
      ...
      <div className="mt-5 flex flex-wrap justify-center gap-1.5">
        {qs.map((q) => (
          <button key={q} onClick={() => onPick(q)} className="rounded-full border border-border bg-surface px-3 py-1.5 text-xs text-text-secondary hover:border-text-muted hover:text-foreground">
            {q}
          </button>
        ))}
      </div>
      ...
      <div className="space-y-2">
        {steps.map((s, i) => (
          <div key={s} className="flex items-center gap-2 text-xs text-text-secondary">
            <div className="h-1.5 w-1.5 rounded-full bg-primary" style={{ animation: `pulse 1.5s ${i * 0.3}s infinite` }} />
            {s}
          </div>
        ))}
      </div>
    ...
    <aside className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-surface p-4">
        ...
        <p className="mt-2 text-xs leading-relaxed text-text-secondary">
          Every claim in an answer links to a page and paragraph. Open the source to verify context and quoted text.
        </p>
      </div>
    </aside>
  );
